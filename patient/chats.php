...
<?php
  ...
  echo"<div class='row'>
    <div class='col-md-4 border-right'>";
      ...
      $query_chat = mysqli_query($connect,"SELECT id, sender, receiver, LEFT(message, 30), mdt FROM chat WHERE receiver = '$thisSender' GROUP BY sender ORDER BY id DESC");
      while($row_chat = mysqli_fetch_array($query_chat)){

        $query_pat = mysqli_query($connect,"SELECT * FROM users WHERE id = '".$row_chat['sender']."'");
 ...
 $thisReceiver = $_GET['receiver'];
      ...
      $query_rec = mysqli_query($connect,"SELECT * FROM users WHERE id = '".$_GET['receiver']."'");
          ...
          <div class='col-12'>
            <form method='POST' action='../server.php'>
              <div class='chat-box-tray'>
                ...
                <button name='sendMessagePat' style='border: none; background: transparent;'><i class='material-icons'>send</i></button>
              </div>
            </form>
          </div>
        </div>
