<?php session_start();
  if (!isset($_SESSION['adminUserToken'])) {
    ...
    $query = mysqli_query($connect, "SELECT * FROM patients WHERE patient_code = '".$_GET['patId']."'");
            ...
            echo"<form method='POST' action='../server.php'>
              <div class='row'>
                <div class='col-xl-12 col-sm-6'>
                  <div class='form-group mb-3'>
                    <input type='hidden' name='pat_id' value='".$data['patient_code']."'>
                    <label class='mb-2'>Enter descriptive feedback on patient with code ".$_GET['patId']."</label>
                    <textarea name='feed_text' class='form-control' placeholder='Enter feedback' required=''></textarea>
                  </div>
                </div>
              </div>

              <button type='submit' name='addFeedBack' class='btn btn-primary'>Save Changes</button>
            </form>";
          ?>

        <div class="col-xl-12 col-sm-12 mb-3" style="overflow-x: auto;">
          <br>
          <h4>Recently Added Feed Backs</h3>
          
          <?php

            $query = mysqli_query($connect, "SELECT * FROM feedbacks WHERE patientid = '".$_GET['patId']."' ORDER BY feedid DESC");
            while($data = mysqli_fetch_array($query)){
              echo"<div class='alert alert-secondary' role='alert'>
                <b>".date('d/m/Y H:i', $data['fdatetime'])."</b><br>
                ".$data['feedback']."
                </div>";
            } 
          ?>
